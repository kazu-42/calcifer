...
| Claude provider-managed profiles | Design verified; implementation pending | Future Linux lane uses official `claude auth login/status/logout` under one `CLAUDE_CONFIG_DIR`; Windows awaits ACL proof and macOS awaits a documented config-scoped Keychain isolation contract |
